...
public class Main {
	...
	static int bfs(List<Integer> temp) {
		...
		while(!q.isEmpty()) {
			int x = q.poll();
			for(int k:a[x]) {
				if(!(check[k]==false&&temp.contains(k))) continue;
				check[k] = true;
				q.offer(k);
			}
		}
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=1;i<(1<<n)-1;i++) {
			...
			for(int j=0;j<n;j++) {
				if((i&(1<<j))!=0) one.add(j+1);
				else two.add(j+1);
			}
			int one_sum = bfs(one);
			int two_sum = bfs(two);
			if(one_sum==-1||two_sum==-1) continue;
			int diff = Math.abs(one_sum-two_sum);
