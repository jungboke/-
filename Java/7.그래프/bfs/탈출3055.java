...
public class Main {
	...
	static String bfs(int sx, int sy, int ex, int ey) {
		...
		for(int i=0;i<water.size();i++) {
			int wx = water.get(i)[0]; int wy = water.get(i)[1];
			wq.offer(new int[] {wx,wy});
			w_check[wx][wy] = true;
		}
		while(!q.isEmpty()) {
			int size = q.size();
		...
		if(dist[ex][ey]==0) return "KAKTUS";
		else return Integer.toString(dist[ex][ey]-1);
	}
